...
def draw_text(text, width, height, fontsize):
    ...
    length = canvas(device).textlength(text)
    ...
    print('x', x, 'y',y)
    if w <= width:
        draw_it(text, font, x, y)
    else:
        w_roll_draw(text, w, y, width, font)

def draw_it(text, font, x, y):
...
def w_roll_draw(text, w, y, width, font, interval=0.004, repeat=2):
    s = math.ceil(w) - width
    for r in range(repeat):
        for i in range(s):
            draw_it(text, font, -1 * i, y)
            time.sleep(interval)
        
        for i in range(s):
            draw_it(text, font, i-s, y)
            time.sleep(interval) 
...
if __name__ == '__main__':
    ...
    frames = []
    with Image.open("/home/pi/pp/bad_apple_128x32.gif") as img:
    ...
    while True:
        ...
        draw_text("shao shi tao is a good man valy good", device.width, device.height, 12)
        time.sleep(2)
